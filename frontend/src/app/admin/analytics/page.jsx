...
export default function AnalyticsDashboard() {
  ...
  const salesByType = [
    { type: "Local Data Only", sales: 2100, color: "bg-[#3a7d71]" },
    { type: "Regional Data", sales: 1200, color: "bg-[#ec5b13]" },
    { type: "Global (Data + Voice)", sales: 542, color: "bg-blue-500" },
  ];
            ...
            {salesByType.map((item) => (
              <div key={item.type}>
                ...
                <div className="w-full bg-gray-100 rounded-full h-3">
                  <div className={`${item.color} h-3 rounded-full`} style={{ width: `${(item.sales / 3842) * 100}%` }}></div>
                </div>
              </div>
            ))}
          </div>
