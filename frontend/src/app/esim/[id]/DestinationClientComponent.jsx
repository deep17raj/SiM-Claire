...
import { useRouter } from "next/navigation"; // Removed useParams, we get it from props now!
...
// 🌟 ACCEPT PROPS HERE (No more mapping logic needed!)
export default function DestinationClientComponent({ matchedCountry, urlSlug }) {
    ...
    // Directly use the data passed from the Server
    const backendDestinationID = matchedCountry ? matchedCountry.destinationID : null;
    const countryInfo = matchedCountry
        ? { name: matchedCountry.destinationName, iso: matchedCountry.isoCode }
        : { name: "Destination", iso: "UN" };

    useEffect(() => {
        const fetchPlans = async () => {
            try {
                const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/get/sims/${backendDestinationID}?countryCode=${currency}`);
        ...
        if (backendDestinationID && !isCurrencyLoading) {
            fetchPlans();
        } else if (!matchedCountry && urlSlug) {
            setError("Destination not found.");
            setLoading(false);
        }
    }, [backendDestinationID, currency, isCurrencyLoading, matchedCountry, urlSlug]);
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#fafafa]">
            <div className="text-center bg-white p-10 rounded-2xl shadow-sm">
                ...
                <button onClick={() => router.push('/destination')} className="cursor-pointer text-brand hover:underline font-medium">Return to Destinations</button>
            </div>
        </div>
    );
}
                ...
                {displayedPlans.map((plan) => (
                    <div key={plan.id} className="bg-white rounded-[20px] p-5 border border-gray-200 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col">
                        ...
                        <div className="flex gap-3 mb-4 mt-auto">
                            {/* 🌟 Pass the CORRECT backend destination ID to checkout 🌟 */}
                            <button
                                onClick={() => router.push(`/checkout/${plan.id}?dest=${backendDestinationID}`)}
...
// ... (Keep the rest of your Filter/Sort/UI code exactly the same here) ...